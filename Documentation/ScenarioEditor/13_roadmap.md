...
- [x] **PoC-G: Tauri 2 scaffold + Rust FS commands + dev mode** — `packages/tauri/` に Rust crate (Cargo.toml + tauri.conf.json + capabilities/default.json + src/) を配置。`adapter-tauri` (TS) が呼ぶ `ss_fs_register/list/read/read_bytes/write/write_bytes/delete/exists` を Rust で実装、`assertSafePath()` の Rust 版で path traversal を入口で拒否 (4 件の cargo test)。CI に `cargo check + cargo test` を 3 OS マトリクスで追加。**残: Phase 3 で実機 build 検証 + code signing + auto update + tauri-plugin-dialog (フォルダピッカー)**
